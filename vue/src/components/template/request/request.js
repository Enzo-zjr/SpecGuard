 ...
 *@description  阅卷目录request  （请求）相关 【注：forbidGlobal（是否禁用loading的加载）参数，需要一起调用的接口给请求时间最长的接口设置为false，其余都设置为true，不然loading会被提前关闭！】
        ...
        requestUserData(queryData) {
            ...
            return new Promise((resolve, reject) => {
                ...
                window.fdGlobal.ajax({
                    ...
                    // 默认值是json
                    responseType: 'json',
                    // 是否禁用loading的加载（每个页面把请求时间最长的接口设置为false，其余设置为true）
                    forbidGlobal: true
                }).then((data) => {
